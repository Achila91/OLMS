...
if (isset($_SESSION['name'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        error_reporting(0);
        foreach ($course_id as $batch) {

            if (sizeof($batch) > 1)
            {
            

            //$query = mysqli_query($con, "insert into t_time (co_name) value ('$batch')");
            $query = mysqli_query($con, "insert into t_time (le_name,username,course,co_name,s_time,e_time,to_time,approval,f_approval,g_approval,coverage,rdate,as_id) value('$nicpost','$name',(select c.c_name from course c, student_course_batch b where b.batch_remarks='$batch' and b.scb_course_id=c.course_id),'$batch','$stime','$etime',TIME_TO_SEC(TIMEDIFF(\"$etime\", \"$stime\")),'Not Approved','Not Approved','Not Approved','$coverage','$rdate',(select id from c_assign where username=\"$name\" and c_name=\"$course_id\"))");
            $query3 = mysqli_query($con, "update c_assign set hr=hr-TIME_TO_SEC(TIMEDIFF(\"$etime\", \"$stime\")) where c_name='$batch' and username='$name'");
...
<body class="hold-transition sidebar-mini layout-fixed">
    ...
    <div class="wrapper" style="padding-left: 20%;">
        ...
        <div>
            ...
            $connect = mysqli_connect("localhost", "root", "", "ectc_lec");
                ...
                <table id="table1" ; cellspacing="5px" cellpadding="5%" ; align="center">
                    <tr>
                        ...
                        <td>
                            ...
                            ?>
                                <input type="text" id="search-box" name="search-box" value="<?php echo $row['f_name']; ?>" readonly />
                            <?php } ?>
                        ...
                        <td>
                            ...
                            if (mysqli_num_rows($sql)) {
                                $select = '<select  multiple id="mySelect" onchange="myFunction()" class="form-select" name="course_id[]" required><option selected disabled value="">-- Please Select --</option>';
        ...
        <div class="row col-md-12 ">
            ...
            <table class="table table-striped table-bordered" id="" class="display" width="96%" cellspacing="0">
                ...
                <?php
                $ret = mysqli_query($con, "select course,s_time , e_time , to_time , co_name,rdate,coverage from t_time where username='$name' and approval!='Approved'");
                while ($row = mysqli_fetch_array($ret)) {
