...
if (isset($_POST['submit'])) {
   ...
   // $rem_pay = $_POST['rem_pay'];
    ...
    $month=$_GET['month'];
    // $year=$_GET['year'];
    $query = mysqli_query($con, "insert into lec_pay (l_name,course,c_name,tot,rate,pay,paid_for,sy_date,as_id,documents) values('$l_name','$course','$c_name','$tot','$rate','$pay','$month','$pay_date','$id','$url')");
    if ($query) {
 ...
 <div class="content-wrapper">
    ...
    <?php
                                    ...
                                    // $ret=mysqli_query($con,"select SEC_TO_TIME(SUM(TIME_TO_SEC(t.to_time))) as total, c.*, l.rem_pay from t_time t, c_assign c, lec_pay l where MONTH(t.rdate)='$month' and YEAR(t.rdate)='$year' and t.as_id=$id and c.id=t.as_id and l.as_id=t.as_id and t.approval='Approved' and l.pay_month='$month' and l.pay_year='$year'");
                                                                    $ret=mysqli_query($con,"select SUM(t.to_time) as total, c.l_name, c.c_name, c.rate, c.course from t_time t, c_assign c where t.rdate like '$month%' and t.as_id=$id and t.approval='Approved' and c.id=t.as_id");
                                    ...
                                    ?>
  <fieldset>
  <h3 class="" style="padding-left:10%">Add Payments - <?php echo $month; ?></h3><br><br>
  <div class="control-group">
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <input type="number" min="0" id="pay" name="pay" style="width: 400px; height:40%;" placeholder="Enter the payment" class="input-xlarge" required >
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <input type="url" id="url" name="url" style="width: 400px; height:40%;" placeholder="Enter the url of documents" class="input-xlarge" required >
...
<table class="table table-striped table-bordered" id="" width="80%" cellspacing="0">
            ...
            <?php
             $id=$_GET['id'];
            $ret = mysqli_query($con, "select * from t_time where as_id=$id and approval='Approved' and rdate like '$month%' order by rdate asc");
