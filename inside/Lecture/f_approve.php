	...
	<div class="wrapper">
		...
		?>
			...
			<table class="table table-striped table-bordered" id="" width="80%" cellspacing="0">
				...
				// $ret = mysqli_query($con, "select * from c_assign where c_assign.id IN(select as_id from t_time) and c_assign.id NOT IN(select as_id from lec_pay where approve='' and rem_pay!=0)");
				//$ret = mysqli_query($con, "select * from c_assign c, lec_pay l where c.status = 'T' and MONTH(c.from_date) <= MONTH(CURRENT_DATE()) AND YEAR(c.from_date) <= YEAR(CURRENT_DATE()) and MONTH(c.to_date) >= MONTH(CURRENT_DATE()) AND YEAR(c.to_date) >= YEAR(CURRENT_DATE()) and c_assign.id IN(select as_id from t_time) and l.approval!='Approved' and c.id=l.as_id");
				$ret = mysqli_query($con, "(select DISTINCT DATE_FORMAT(`rdate`,'%Y-%m') AS Month, t.le_name, t.co_name, c.course, c.rate, t.as_id from t_time t, c_assign c where c.id=t.as_id and t.approval='Approved' and pay!='1' and DATE_FORMAT(`rdate`,'%Y-%m')='$mydate') order by t.le_name asc");
				// c_assign.id NOT IN(select as_id from lec_pay)
			...
			?>
				...
				<table class="table table-striped table-bordered" id="" width="80%" cellspacing="0">
					...
					$ret = mysqli_query($con, "select * from lec_pay where finalize IS NULL and paid_for like '$mydate%' order by l_name asc");
					while ($row = mysqli_fetch_array($ret)) {
